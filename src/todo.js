...
const createTodo = (
  title = "",
  description = "",
  project = loadLocalStorage()[0].title,
  dueDate,
  priority = 4,
  selected = false
) => {
...
const addTaskToStorage = () => {
  const get = () => {
    ...
    const taskProjectTitle = document.querySelector(
...
const getDueDate = (input) => {
  ...
  const dueDate = new Date(
    valueAsDate.getTime() + valueAsDate.getTimezoneOffset() * 60000
  );
  return dueDate;
};
...
const formatDueDate = (dueDateObject) => {
  ...
  const readableDueDate = isDueDateThisYear
    ? format(dueDateObject, "MMM dd")
    : format(dueDateObject, "MMM dd yyyy");
...
const addDueDateInput = (e) => {
  ...
  if (
    container.children[2] !== undefined &&
    container.children[2].id === "due-date-input"
  )
    return;
...
const openRemoveProjectConfirmationMenu = (e) => {
  ...
  confirmDeletionIsolatedContainer(projectList[listId]).appendToParent(
    `#project-list > li[data-list-id="${listId}"]`
  );
};
...
const selectProject = (e, checkIfEventInput = true) => {
  ...
  if (checkIfEventInput) {
    ...
    const projectListItem = document.querySelector(
      `.project-list-item[data-list-id="${projectListId}"`
    );

    if (
      projectList[projectListId] === undefined ||
...
export {
  ...
  removeTask,
  addProject,
  openRemoveProjectConfirmationMenu,
  removeProject,
  selectProject,
  getActiveProject,
  addDueDateInput,
